...
Installs [mnemon](https://github.com/mnemon-dev/mnemon) in the agent container image. On each container start, `mnemon setup` registers Codex hooks that surface relevant memory before the agent responds and store new insights after each turn. Memory is written to the per-agent-group `.claude-shared/` mount and survives container restarts.
...
```bash
pnpm exec tsx scripts/q.ts data/v2.db "SELECT id, agent_provider FROM agent_groups;"
```
...
`MNEMON_DATA_DIR` points into the per-agent-group `.claude-shared/` mount so memory persists across container restarts. No extra volume mounts needed.
...
mnemon setup --target Codex --yes --global 2>/tmp/mnemon-setup.log
...
`2>/tmp/mnemon-setup.log` captures the setup output in-container for diagnosis without touching the handshake stream.
...
$RUNTIME run --rm --entrypoint mnemon nanoclaw-agent:latest --version
```
...
```bash
RUNTIME="${CONTAINER_RUNTIME:-docker}"
$RUNTIME logs $($RUNTIME ps --filter name=nanoclaw-v2 --format '{{.Names}}' | head -1) 2>&1 | grep -i mnemon
```
...
```bash
RUNTIME="${CONTAINER_RUNTIME:-docker}"
$RUNTIME exec $($RUNTIME ps --filter name=nanoclaw-v2 --format '{{.Names}}' | head -1) \
...
**What this means for the agent:** The agent (running inside OpenCode) can call `mnemon recall`, `mnemon remember`, `mnemon link`, and `mnemon status` via its bash tool. mnemon writes its graph to `$MNEMON_DATA_DIR`, which is in the per-agent-group `.claude-shared/` mount — so memory persists across container restarts.
...
Mnemon writes to `/home/node/.claude/mnemon/` inside the container, which maps to the per-agent-group `.claude-shared/` directory on the host. To find the exact host path:

```bash
RUNTIME="${CONTAINER_RUNTIME:-docker}"
$RUNTIME inspect $($RUNTIME ps --filter name=nanoclaw-v2 --format '{{.Names}}' | head -1) \
...
```bash
RUNTIME="${CONTAINER_RUNTIME:-docker}"
$RUNTIME exec <container> sh -c 'ls -la $MNEMON_DATA_DIR'
```

If the directory is empty after conversations, the mount is missing or the path is wrong. Check the host mount with the runtime inspect command above.
...
```bash
RUNTIME="${CONTAINER_RUNTIME:-docker}"
$RUNTIME exec <container> cat /home/node/.claude/settings.json
```
...
```bash
RUNTIME="${CONTAINER_RUNTIME:-docker}"
$RUNTIME exec -it <container> mnemon setup --target Codex --yes --global
```
