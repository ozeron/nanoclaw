---
name: add-codex
description: Use Codex (CLI + AppServer) as the full agent provider — planning, tool orchestration, native compaction, MCP tools, session resume — for OpenAI-powered sessions with `OPENAI_API_KEY` or `~/.codex/auth.json`. Per-group via `agent_provider`. Distinct from using OpenAI as an MCP tool (where Codex remains the planner).
---

# Codex agent provider

NanoClaw runs agents in a long-lived **poll loop** inside the container. The backend is selected from `sessions.agent_provider` first, then `container_configs.provider`, then the default `codex`.

This fork ships the `codex` provider as the default. Use this skill as a verify/repair path: if any Codex provider file is missing, copy it from the providers branch, wire the barrels, ensure the Dockerfile installs the Codex CLI, and rebuild the image.

The Codex provider runs `codex app-server` as a child process and speaks JSON-RPC over stdio. That gives it native session resume, streaming events, MCP tool access, and `thread/compact/start` compaction.
...
**(a)** Ensure the `ARG CODEX_VERSION=...` entry exists in the "Pin CLI versions" block:

```dockerfile
ARG CODEX_VERSION=0.124.0
```

**(b)** Add a new standalone `RUN` block for the Codex CLI in the per-tool install section. The Dockerfile splits each global CLI into its own layer for cache granularity — keep that pattern; do not collapse them into a single combined `pnpm install -g` call:
...
Use the DB-backed config, not hand edits to `groups/<folder>/container.json`. The host materializes `container.json` from `container_configs` at spawn time, so manual edits are overwritten.

```bash
ncl groups config update --id <agent-group-id> --provider codex
ncl groups config update --id <agent-group-id> --model gpt-5.4-mini
```

`CODEX_MODEL` in `.env` is a process-wide fallback. For different models per group, set `container_configs.model` through `ncl groups config update --model`; the runner passes that value to the Codex provider.

Extra MCP servers come from `container_configs.mcp_servers` and are managed through `ncl groups config add-mcp-server/remove-mcp-server` or the self-mod MCP tool. The runner merges those servers into the provider config on startup.
...
If the agent hangs or errors, check `~/.codex/auth.json` exists on the host (Option A) or that `OPENAI_API_KEY` is forwarding correctly (Option B) — `${CONTAINER_RUNTIME:-docker} exec` into a running container and `env | grep -i openai` to confirm.
