...
Then find the per-tool pnpm global-install section and add a new standalone block before `# ---- ncl CLI wrapper`:
...
**No `TOOL_ALLOWLIST` edit needed.** The Codex provider derives MCP access dynamically from each group's `mcpServers` map, so registering `gmail` in Phase 3 automatically exposes `mcp__gmail__*`. Earlier versions of this skill instructed a static `TOOL_ALLOWLIST` edit — that's now redundant.
