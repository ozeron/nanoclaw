...
Read `.agents/skills/add-karpathy-llm-wiki/llm-wiki.md` — this is the full LLM Wiki idea as written by Karpathy. Understand it thoroughly before proceeding. Summarize the core idea to the user briefly, then discuss what they want to build.
...
### 3c. Group prompt instructions

Edit the group's `CLAUDE.local.md` (not v1 `AGENTS.md`) to add a wiki section. This is critical — it's what turns the agent into a wiki maintainer. It should:
...
Codex has built-in `WebFetch`, but it returns a summary, not the full document. For wiki ingestion of a URL where the full text matters, the container skill and `CLAUDE.local.md` should instruct Codex to use bash commands to download full files instead. For example:

```bash
curl -sLo sources/filename.pdf "<url>"
```

If the document is a webpage, then Codex can use fetch or `agent-browser` to open the page and extract full text if available. The container skill and `CLAUDE.local.md` should note this so Codex gets full content for sources rather than summaries.
