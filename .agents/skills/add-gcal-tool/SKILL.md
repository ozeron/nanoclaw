...
**No `TOOL_ALLOWLIST` edit needed.** The Codex provider derives MCP access dynamically from each group's `mcpServers` map, so registering `calendar` in Phase 3 automatically exposes `mcp__calendar__*`. Earlier versions of this skill instructed a static `TOOL_ALLOWLIST` edit — that's now redundant.
...
$RUNTIME ps -q --filter 'name=nanoclaw-v2-' | xargs -r $RUNTIME kill
```
