...
describe('createProvider (codex)', () => {
  ...
  it('resolves model from group config before env and default', () => {
    expect(resolveCodexModel({ model: 'gpt-5.4', env: { CODEX_MODEL: 'gpt-5.4-mini' } })).toBe('gpt-5.4');
    expect(resolveCodexModel({ env: { CODEX_MODEL: 'gpt-5.4-mini' } })).toBe('gpt-5.4-mini');
    expect(resolveCodexModel()).toBe('gpt-5.4-mini');
  });
});
