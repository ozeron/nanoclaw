...
export function resolveCodexModel(options: ProviderOptions = {}): string {
  return options.model ?? (options.env?.CODEX_MODEL as string | undefined) ?? 'gpt-5.4-mini';
}
...
export class CodexProvider implements AgentProvider {
  ...
  constructor(options: ProviderOptions = {}) {
    ...
    this.model = resolveCodexModel(options);
  }
